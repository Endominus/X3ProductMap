public class Factory {
	...
	public Factory(int defaultType, char race, char size) {
		Factory factoryTemplate = Controller.MASTER_FACTORY_LIST
				.get(defaultType);
		this.resDemanded = factoryTemplate.getResources().clone();
		
		this.resStockpiled = this.resDemanded.clone();
		for (int[] res : this.resStockpiled)
		{
			res[1] = 0;
		}
	...
	private void AdjustResources(int multiplier, boolean divide) {
		...
		for (int i = 0; i < this.resDemanded.length; i++) {
			this.resDemanded[i][1] = this.resDemanded[i][1] * multiplier / divisor;
		}
	...
	public double[] DemandRatio(int id)
	{
		...
		for (i = 1; i < this.resDemanded.length; i++)
		{
			...
			{
				ratio = (-1.0)*this.resStockpiled[i][1]/this.resDemanded[i][1];
				break;
			}
		}
		
		return new double[] { ratio, this.resStockpiled[i][0] + this.resDemanded[i][0] };
	}
	...
	public String toString() {
		...
		for (int i = 0; i < this.resDemanded.length; i++) {
			s = String.format("\t\t%s: %d\n",
					Controller.MASTER_RESOURCE_LIST.get(this.resDemanded[i][0]),
